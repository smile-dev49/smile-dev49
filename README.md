<div align="center">
  <h1>
    <a href="https://t.me/so_so_funny" style="color: blue; font-size:25px; text-decoration: none;">
      ⚡ Full Stack Developer & AI Engineer ⚡
    </a>
  </h1>
  <p>
    <i>Building intelligent, scalable applications at the intersection of web development and artificial intelligence</i>
  </p>
</div>
<br>

### 💎 ABOUT ME

- **Full Stack Developer** with expertise across modern web technologies ✨
  - Frontend: React, Next.js, Angular, TypeScript, Tailwind CSS
  - Backend: Python (Django, FastAPI), Node.js, RESTful APIs, GraphQL
  - Database: PostgreSQL, MongoDB, Redis
  
- **AI/ML Engineer** specializing in intelligent systems 🤖
  - Machine Learning & Deep Learning with TensorFlow, PyTorch, Scikit-learn
  - Natural Language Processing (NLP) and Computer Vision
  - LLM Integration (OpenAI, Anthropic, Hugging Face)
  - AI-powered application development and deployment
  
- **Technical Expertise** 💯
  - Building robust, scalable RESTful APIs and microservices
  - Developing high-performance web applications with modern frameworks
  - Implementing ML models and AI solutions in production environments
  - Database design, optimization, and API integration
  - Cloud deployment (AWS, GCP, Azure) and DevOps practices

- Passionate about leveraging AI to solve real-world problems 🚀
- Committed to writing clean, maintainable code and delivering exceptional user experiences 👨‍💻

### ⚔ TECHNOLOGY STACK

#### Frontend Development
<table align="center">
  <tr>
    <td align="center" width="150">
      <img src="https://techstack-generator.vercel.app/react-icon.svg" height="50" alt="React" />
      <br>React
    </td>
    <td align="center" width="150">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nextjs/nextjs-original.svg" height="50" alt="Next.js" />
      <br>Next.js
    </td>
    <td align="center" width="150">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/angularjs/angularjs-original.svg" height="50" alt="Angular" />
      <br>Angular
    </td>
    <td align="center" width="150">
      <img src="https://techstack-generator.vercel.app/ts-icon.svg" height="50" alt="TypeScript" />
      <br>TypeScript
    </td>
  </tr>
</table>

#### Backend Development
<table align="center">
  <tr>
    <td align="center" width="150">
      <img src="https://techstack-generator.vercel.app/python-icon.svg" height="50" alt="Python" />
      <br>Python
    </td>
    <td align="center" width="150">
      <img src="https://techstack-generator.vercel.app/django-icon.svg" height="40" alt="Django" />
      <br>Django
    </td>
    <td align="center" width="150">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/fastapi/fastapi-original.svg" height="50" alt="FastAPI" />
      <br>FastAPI
    </td>
    <td align="center" width="150">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/nodejs/nodejs-original.svg" height="50" alt="Node.js" />
      <br>Node.js
    </td>
  </tr>
</table>

#### AI & Machine Learning
<table align="center">
  <tr>
    <td align="center" width="150">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/tensorflow/tensorflow-original.svg" height="50" alt="TensorFlow" />
      <br>TensorFlow
    </td>
    <td align="center" width="150">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/pytorch/pytorch-original.svg" height="50" alt="PyTorch" />
      <br>PyTorch
    </td>
    <td align="center" width="150">
      <img src="https://upload.wikimedia.org/wikipedia/commons/0/05/Scikit_learn_logo_small.svg" height="50" alt="Scikit-learn" />
      <br>Scikit-learn
    </td>
    <td align="center" width="150">
      <img src="https://huggingface.co/front/assets/huggingface_logo-noborder.svg" height="50" alt="Hugging Face" />
      <br>Hugging Face
    </td>
  </tr>
</table>

#### Database & Cloud
<table align="center">
  <tr>
    <td align="center" width="150">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/postgresql/postgresql-original.svg" height="50" alt="PostgreSQL" />
      <br>PostgreSQL
    </td>
    <td align="center" width="150">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/mongodb/mongodb-original.svg" height="50" alt="MongoDB" />
      <br>MongoDB
    </td>
    <td align="center" width="150">
      <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/redis/redis-original.svg" height="50" alt="Redis" />
      <br>Redis
    </td>
    <td align="center" width="150">
      <img src="https://techstack-generator.vercel.app/aws-icon.svg" height="50" alt="AWS" />
      <br>AWS
    </td>
  </tr>
</table>

### 📊 GitHub Stats

<div align="center">
  <img src="https://github.com/devafortun/github-contribution-grid-snake.svg/raw/main/github-contribution-grid-snake.svg" alt="Snake gif" style="max-width: 100%;">
</div>
